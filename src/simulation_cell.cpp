...
simulation_cell::simulation_cell(const string& path_strctr)
{
	std::filesystem::path path_structure;
	try {
		path_structure = std::filesystem::canonical(path_strctr);
	}
	catch (const std::filesystem::filesystem_error & e) {
		std::cout << e.what() << std::endl;
		exit(EXIT_FAILURE);
	}
	std::ifstream file(path_strctr);
	if (!file.is_open()) {
	...
	{
		cerr << RED << "Failed to construct the simulation cell class from file: " << path_structure.filename() << endl;
		cerr << "The program is aborted." << RESET << endl;
		exit(EXIT_FAILURE);
	}
